...
class PizzaRequestInput(BaseModel):
    ...
    This must match the structure of a single row from the training data.
    """
    ...
    # --- Features present in raw data that the pipeline expects to drop ---
    # We include them here to ensure the input DataFrame for prediction
    # has the exact same columns as the DataFrame used for training.
    # We can make them optional with a default value if they are not critical.
    ...
    class Config:
        # This will generate a nice example in the /docs UI
        json_schema_extra = {
            "example": {
                "request_id": "t3_w5491",
